...
export default function XSSPage() {
  const xssTools = toolkitData.filter(tool => tool.category === 'Web Exploitation' && tool.title.toLowerCase().includes('xss'));

  const [searchTerm, setSearchTerm] = useState('');
  const filteredTools = xssTools.filter(tool =>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-8">
              ...
              <div className="hidden md:flex space-x-6">
                ...
                <Link href="/xss" className="text-foreground hover:text-primary transition-colors">
                  XSS Payloads
                </Link>
                <Link href="/sql-injection" className="text-muted-foreground hover:text-primary transition-colors">
                  SQL Injection
                </Link>
              </div>
            </div>
            <div className="text-sm text-muted-foreground">
              XSS Payloads and Scripts
            </div>
          </div>
        </div>
      </nav>
      ...
      <section className="py-12 px-4">
        <div className="container mx-auto text-center">
          ...
          <p className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
            Explore various XSS payloads and scripts for testing and educational purposes.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-6xl">
          {filteredTools.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              No XSS payloads found matching your criteria.
            </div>
          ) : (
      ...
      <footer className="border-t border-border/40 py-8 px-4 mt-12">
        <div className="container mx-auto text-center">
          <p className="text-muted-foreground text-sm">
            FSociety Toolkit - XSS Payloads and Scripts
          </p>
